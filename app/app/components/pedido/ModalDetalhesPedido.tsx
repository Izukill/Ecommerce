...
import ModalZoom from "../layout/ModalZoom";
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
        <div className="bg-neutral-900 border-t-4 border-[#C2AE82] rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
              ...
              {pedidoSelecionado.itens && pedidoSelecionado.itens.length > 0 ? (
                <ul className="space-y-3">
                  {pedidoSelecionado.itens.map((item, idx) => {
                    const imagemExibicao = item.imagemUrl || item.imagemCapa;
                          ...
                          <div className="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 bg-neutral-900 rounded-md border border-neutral-700 overflow-hidden flex items-center justify-center">
                            {imagemExibicao ? (
                              <img
                                ...
                                onClick={() => setImagemZoom(imagemExibicao)}
          ...
          <div className="p-4 sm:p-6 border-t border-neutral-800 bg-black/50 rounded-b-xl flex flex-col-reverse sm:flex-row justify-between items-center gap-4 sm:gap-0">
            ...
            <div className="flex flex-col w-full sm:w-auto justify-center sm:justify-end bg-neutral-900 sm:bg-transparent p-3 sm:p-0 rounded-lg border border-neutral-800 sm:border-0 gap-1.5">

              {pedidoSelecionado.freteFixo !== undefined && pedidoSelecionado.freteFixo !== null && (
                <div className="flex items-center justify-between sm:justify-end gap-3 w-full">
                  <span className="text-[10px] sm:text-xs font-bold text-gray-500">+ Frete</span>
                  <span className="text-sm sm:text-base font-bold text-gray-300">
                    {formatarMoeda(pedidoSelecionado.freteFixo)}
                  </span>
                </div>
              )}

              <div className="flex items-center justify-between sm:justify-end gap-3 w-full pt-1.5 sm:pt-0 border-t border-neutral-800 sm:border-0">
                <span className="text-xs sm:text-sm font-bold text-gray-400 uppercase">Total do Pedido</span>
                <span className="text-xl sm:text-2xl font-extrabold text-[#C2AE82]">
